    ...
    if not recomp:
        ...
        if band == 'custom_multi' or band == 'custom_multi_symmetrical' or band == 'custom_multi_custom':
            ...
            if family == 'Other':
                ...
                strategy = col3.selectbox('Select the strategy', get_strategy_families(t0=t_start, environment='production')
                [family], index=11)
        ...
        if math.isnan(taker_fee):
            taker_fee = 0.0003
        spot_fee = fee_1.number_input('Spot Fee', min_value=-1.0, value=taker_fee, max_value=1.0, step=0.0001, format="%.6f")
        swap_fee = fee_2.number_input('Swap Fee', min_value=-1.0, value=maker_fee, max_value=1.0, step=0.0001, format="%.6f")
        ...
        ws_swap, api_swap, ws_spot, api_spot = set_latencies_auto(exchange_swap=exchange_swap, exchange_spot=exchange_spot)
        ...
        latency_spot_balance = col_lat2.number_input('Latency Spot Balance', min_value=0, value=api_spot, max_value=1000)

        col_ts1, col_ts2 = st.columns(2)
        max_trade_volume = col_ts1.number_input('Max Trade Volume', min_value=0, value=3000, max_value=100000, step=1000)
        ...
        if band == 'percentage_band':
            ...
            entry_delta_spread_max = col_max.number_input('entry_delta_spread max value', min_value=-100.0,
                                                          max_value=+100.0)
            entry_delta_spread_step = col_step.number_input('entry_delta_spread step', min_value=-100.0, max_value=+100.0)

            if band != 'custom_multi_symmetrical':
                exit_delta_spread_min = col_min.number_input('exit_delta_spread min value', min_value=-100.0, max_value=+100.0)
                exit_delta_spread_max = col_max.number_input('exit_delta_spread max value', min_value=-100.0, max_value=+100.0)
                exit_delta_spread_step = col_step.number_input('exit_delta_spread step', min_value=-100.0, max_value=+100.0)
        ...
        if check:
            if band == 'custom_multi':
                ...
                for ws in [1800, 2000, 2500, 2700, 3300, 3500, 4000, 4200, 4400]:
                    for ens in np.arange(entry_delta_spread_min, entry_delta_spread_max, entry_delta_spread_step):
                        for exs in np.arange(exit_delta_spread_min, exit_delta_spread_max, exit_delta_spread_step):
                            if (int(ens*10) % 2 == 0 and int(exs*10) % 2 == 1) or (int(ens*10) % 2 == 1 and int(exs*10) % 2 == 0):
                                continue
