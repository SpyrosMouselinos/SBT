...
from src.common.queries.queries import get_strategy_families, get_symbol, get_strategy_influx, get_exhange_names, \
    get_percentage_band_values, get_entry_exit_bands
from src.common.queries.queries import get_band_values, get_executions
import hiplot as hip
import logging

from src.common.utils.trader_utils import get_data_for_trader
from old_code.maker_maker.MakerMakerMasterFile import SimulationBase
...
def streamlit_maker_maker_trader():
    ...
    if family == 'Other':
        ...
        strategy = col3.selectbox('Select the strategy', get_strategy_families(t0=t_start, environment='production')[family], index=9)
    else:
    ...
    if check:
        ...
        df = get_data_for_trader(t0, t1, exchange_spot, spot_instrument, exchange_swap, swap_instrument,
                                 ...
                                 maker_fee_swap=maker_fee_swap, strategy=strategy, area_spread_threshold=area_spread_threshold,
                                 environment=environment)
        ...
        #                          area_spread_threshold=area_spread_threshold, environment=environment, band_type=band,
        #                          lookback=lookback,
        ...
        t.tic()
        model = SimulationBase(df=df, maker_fee_swap=maker_fee_swap, taker_fee_swap=taker_fee_swap, maker_fee_spot=maker_fee_spot,
                               ...
                               latency_spot=latency_spot, latency_swap=latency_swap, latency_try_post_spot=latency_try_post_spot,
        ...
        if len(model.executions_as_maker) == 0:
            simulated_executions_maker = pd.DataFrame(columns=["timems", "timestamp_swap_executed", "timestamp_spot_executed", "executed_spread", "central_band", "was_trying_to_cancel_spot", "was_trying_to_cancel_swap", "source_at_execution_swap", "dest_at_execution_swap", "source_at_execution_spot", "dest_at_execution_spot", "is_balancing", "is_balancing_spot", "side"])
        else:
            simulated_executions_maker = pd.DataFrame(model.executions_as_maker)
        if len(model.executions_as_taker) == 0:
            model.executions_as_taker = pd.DataFrame(columns=["timems", "timestamp_swap_executed", "timestamp_spot_executed", "executed_spread", "central_band", "was_trying_to_cancel_spot", "was_trying_to_cancel_swap", "source_at_execution_swap", "dest_at_execution_swap", "source_at_execution_spot", "dest_at_execution_spot", "is_balancing", "is_balancing_spot", "side"])
        else:
        ...
        if len(simulated_executions_maker[sim_ex_maker_exit_mask].central_band) > 0 and len(simulated_executions_maker[sim_ex_maker_exit_mask].executed_spread) > 0:
            assert (simulated_executions_maker[sim_ex_maker_exit_mask].executed_spread < simulated_executions_maker[sim_ex_maker_exit_mask].central_band).all(), "Something is wrong in the computation"
        if len(simulated_executions_maker[sim_ex_maker_entry_mask].executed_spread) > 0 and len(simulated_executions_maker[sim_ex_maker_entry_mask].central_band) > 0:
            assert (simulated_executions_maker[sim_ex_maker_entry_mask].executed_spread > simulated_executions_maker[sim_ex_maker_entry_mask].central_band).all(), "Something is wrong in the computation"
        simulated_executions_maker['execution_quality'] = np.abs(simulated_executions_maker.central_band - simulated_executions_maker.executed_spread)
        simulated_executions_maker['is_spot_first'] = simulated_executions_maker.timestamp_spot_executed < simulated_executions_maker.timestamp_swap_executed
        ...
        simulated_executions_taker['execution_quality'][sim_ex_taker_exit_mask] = simulated_executions_taker[sim_ex_taker_exit_mask].central_band - simulated_executions_taker[sim_ex_taker_exit_mask].executed_spread
        simulated_executions_taker['execution_quality'][sim_ex_taker_entry_mask] = simulated_executions_taker[sim_ex_taker_entry_mask].executed_spread - simulated_executions_taker[sim_ex_taker_entry_mask].central_band
        simulated_executions_taker['is_spot_first'] = simulated_executions_taker.timestamp_spot_executed < simulated_executions_taker.timestamp_swap_executed
        ...
        percentage_executions_maker_exit = len(maker_exits) / (len(maker_exits) + len(taker_exits)) if len(maker_exits) + len(taker_exits) > 0 else 0
        percentage_executions_maker_entry = len(maker_entries) / (len(maker_entries) + len(taker_entries)) if len(maker_entries) + len(taker_entries) > 0 else 0
        percentage_executions_maker = (len(maker_entries) + len(maker_exits)) / (len(maker_entries) + len(maker_exits) + len(taker_entries) + len(taker_exits))

        col_res_1, col_res_2, col_res_3 = st.columns(3)
        col_res_1.markdown(f"<b>Percentage Maker Maker exit: {int(percentage_executions_maker_exit * 10000) / 100}%</b>", unsafe_allow_html=True)
        col_res_2.markdown(f"<b>Percentage Maker Maker entry: {int(percentage_executions_maker_entry * 10000) / 100}%</b>", unsafe_allow_html=True)
        col_res_3.markdown(f"<b>Percentage Maker Maker: {int(percentage_executions_maker * 10000) / 100}%</b>", unsafe_allow_html=True)
        col_res_4, col_res_5, col_res_6, col_res_7 = st.columns(4)
        col_res_4.markdown(f"<b>Number of Maker Maker executions entry: {len(maker_entries)}</b>", unsafe_allow_html=True)
        col_res_5.markdown(f"<b>Number of Maker Maker executions exit: {len(maker_exits)}</b>", unsafe_allow_html=True)
        col_res_6.markdown(f"<b>Number of Taker Maker executions entry: {len(taker_entries)}</b>", unsafe_allow_html=True)
        ...
        col_res_8.markdown(f"<b>Avg executed spread entry: {round((taker_entries.executed_spread.sum() + maker_entries.executed_spread.sum()) / (len(taker_entries) + len(maker_entries)), 2)}</b>", unsafe_allow_html=True)
        col_res_9.markdown(f"<b>Avg executed spread exit: {round((taker_exits.executed_spread.sum() + maker_exits.executed_spread.sum()) / (len(taker_exits) + len(maker_exits)), 2)}</b>", unsafe_allow_html=True)
        col_res_10.markdown(f"<b>Avg executed balancing spread entry: {round(taker_entries.executed_spread.mean(), 2)}</b>", unsafe_allow_html=True)
        col_res_11.markdown(f"<b>Avg executed balancing spread exit: {round(taker_exits.executed_spread.mean(), 2)}</b>", unsafe_allow_html=True)
        col_res_12, col_res_13, col_res_14, col_res_15 = st.columns(4)
        col_res_12.markdown(f"<b>Avg execution quality entry: {round((taker_entries.execution_quality.sum() + maker_entries.execution_quality.sum()) / (len(taker_entries) + len(maker_entries)), 2)}</b>", unsafe_allow_html=True)
        col_res_13.markdown(f"<b>Avg execution quality exit: {round((taker_exits.execution_quality.sum() + maker_exits.execution_quality.sum()) / (len(taker_exits) + len(maker_exits)), 2)}</b>", unsafe_allow_html=True)
        col_res_14.markdown(f"<b>Avg execution quality balancing entry: {round(taker_entries.execution_quality.mean(), 2)}</b>", unsafe_allow_html=True)
        col_res_15.markdown(f"<b>Avg execution quality balancing exit: {round(taker_exits.execution_quality.mean(), 2)}</b>", unsafe_allow_html=True)
        col_res_16, col_res_17, col_res_18, col_res_19 = st.columns(4)
        col_res_16.markdown(f"<b>Balancings entry (spot first): {len(taker_entries[taker_entries.is_spot_first])}</b>",unsafe_allow_html=True)
        col_res_18.markdown(f"<b>Balancings exit (spot first): {len(taker_entries[~taker_entries.is_spot_first])}</b>",unsafe_allow_html=True)
        col_res_17.markdown(f"<b>Balancings entry (swap first): {len(taker_exits[taker_exits.is_spot_first])}</b>", unsafe_allow_html=True)
        col_res_19.markdown(f"<b>Balancings exit (swap first): {len(taker_exits[~taker_exits.is_spot_first])}</b>", unsafe_allow_html=True)
        col_res_20, col_res_21, col_res_22, col_res_23 = st.columns(4)
        col_res_20.markdown(f"<b>Maker Maker entry (spot first): {len(maker_entries[maker_entries.is_spot_first])}</b>",unsafe_allow_html=True)
        col_res_22.markdown(f"<b>Maker Maker exit (spot first): {len(maker_entries[~maker_entries.is_spot_first])}</b>",unsafe_allow_html=True)
        col_res_21.markdown(f"<b>Maker Maker entry (swap first): {len(maker_exits[maker_exits.is_spot_first])}</b>", unsafe_allow_html=True)
        col_res_23.markdown(f"<b>Maker Maker exit (swap first): {len(maker_exits[~maker_exits.is_spot_first])}</b>", unsafe_allow_html=True)

        st.markdown("<h3 style='text-align: center; color: grey;'>Descriptive statics of Simulated Maker Maker Executions in the given period</h3>", unsafe_allow_html=True)
        temp_col_1, temp_col_2, temp_col_3 = st.columns(3)
        temp_col_1.text('')
        temp_col_2.dataframe(simulated_executions_maker.describe())
        temp_col_3.text('')
        st.markdown("<h3 style='text-align: center; color: grey;'>Descriptive statics of Simulated Taker Maker Executions in the given period</h3>", unsafe_allow_html=True)
        ...
        if len(simulated_executions_maker) > 0:
            xp_maker = hip.Experiment.from_dataframe(simulated_executions_maker[['was_trying_to_cancel_spot', 'was_trying_to_cancel_swap', 'is_spot_first', 'execution_quality', 'side']])
            st.markdown("<h3 style='text-align: center; color: grey;'>Maker Maker executions</h3>", unsafe_allow_html=True)
            xp_maker.to_streamlit().display()
        if len(simulated_executions_taker) > 0:
            xp_taker = hip.Experiment.from_dataframe(simulated_executions_taker[['was_trying_to_cancel_spot', 'was_trying_to_cancel_swap', 'is_balancing_spot', 'is_spot_first', 'execution_quality', 'side']])
            st.markdown("<h3 style='text-align: center; color: grey;'>Taker Maker executions</h3>", unsafe_allow_html=True)
        ...
        simulated_executions_maker = pd.merge_ordered(band_values[['Central Band', 'timems']], simulated_executions_maker, on='timems')
        ...
        simulated_executions_taker = pd.merge_ordered(band_values[['Central Band', 'timems']], simulated_executions_taker, on='timems')
        ...
        if actual_exists == 'yes':
            ...
            actual_band_values = get_band_values(t0=t_start, t1=t_end, typeb='bogdan_bands', strategy=strategy, environment=environment)
            ...
            actual_executions = actual_executions[(~actual_executions['exit_diff'].isna()) | (~actual_executions['entry_diff'].isna())]
        ...
        st.download_button("Press to Download", simulated_executions_maker.to_csv(), f"Sim_{strategy}_from_{date_range[0]}_to_{date_range[1]}.csv", "text/csv")
        st.download_button("Press to Download", simulated_executions_taker.to_csv(), f"Sim_{strategy}_from_{date_range[0]}_to_{date_range[1]}.csv", "text/csv")
        ...
        if actual_exists == 'yes':
            ...
            fig.add_trace(go.Scatter(
                ...
                y=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'Central Band'],
            ...
            fig.add_trace(go.Scatter(
                x=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'Time'],
                y=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'breakeven_band_entry_balancing_spot'],
            ...
            fig.add_trace(go.Scatter(
                x=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'Time'],
                y=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'breakeven_band_exit_balancing_swap'],
            ...
            fig.add_trace(go.Scatter(
                x=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'Time'],
                y=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'breakeven_band_exit_balancing_spot'],
            ...
            fig.add_trace(go.Scatter(
                ...
                x=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'Time'],
                y=band_values.loc[(~band_values['Central Band'].isna()) & (band_values['Central Band'] <= 200), 'breakeven_band_entry_balancing_swap'],
        ...
        # skipping for now
        if False:
            fig1 = px.histogram(executions_origin[['side', 'previous_state', 'timems']].sort_values(by=['previous_state']),
            ...
            fig2 = px.histogram(executions_origin[['side', 'previous_state', 'timems']].sort_values(by=['previous_state']),
