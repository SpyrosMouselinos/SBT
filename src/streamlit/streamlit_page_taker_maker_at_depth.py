...
from src.common.queries.queries import get_strategy_families, get_symbol, get_strategy_influx, get_exhange_names, \
    get_entry_exit_bands, get_executions, get_band_values
from src.simulations.taker_maker.TakerMakerFunctions import get_data_for_trader
...
def run_simulation():
    ...
    if check:
        ...
        band_values = get_entry_exit_bands(t0=t_start, t1=t_end, strategy=strategy, entry_delta_spread=entry_delta_spread, exit_delta_spread=exit_delta_spread, btype='central_band', environment=environment)
        ...
        df, _ = get_data_for_trader(t_start, t_end, exchange_spot, spot_instrument, exchange_swap, swap_instrument,
                                 ...
                                 entry_delta_spread=entry_delta_spread, band_funding_system="No", generate_percentage_bands=False,
        ...
        model = TakerMakerDeeperLevelSpread(df=df, spot_fee=taker_fee_spot, swap_fee=maker_fee_swap,
                                            ...
                                            environment=environment, exchange_swap=exchange_swap, swap_instrument=swap_instrument,
        ...
        if len(model.cancelled_orders_swap) == 0:
            ...
            for order in model.cancelled_orders_swap:
                temp_cancellations.append([order.timestamp_posted, order.timestamp_cancelled, order.cancelled_to_post_deeper,
            ...
            simulated_cancellations['Time cancelled'] = pd.to_datetime(simulated_cancellations['timestamp_cancelled'], unit='ms')
        ...
        simulated_executions = pd.merge_ordered(band_values[['Central Band', 'Entry Band', 'Exit Band', 'timems']], simulated_executions, on='timems')
        ...
        simulated_executions['execution_quality'][sim_ex_entry_mask] = np.abs(simulated_executions[sim_ex_entry_mask]['Entry Band'] - simulated_executions[sim_ex_entry_mask].executed_spread)
        simulated_executions['execution_quality'][sim_ex_exit_mask] = np.abs(simulated_executions[sim_ex_exit_mask]['Exit Band'] - simulated_executions[sim_ex_exit_mask].executed_spread)
        ...
        total_volume_traded_in_token = (entries.volume_executed / entries.price).sum() + (exits.volume_executed / exits.price).sum()
        ...
        col_res_3.markdown(f"<b>Avg executed spread entry: {round(entries.executed_spread.sum() / len(entries), 2)}</b>", unsafe_allow_html=True)
        col_res_4.markdown(f"<b>Avg executed spread exit: {round(exits.executed_spread.sum() / len(exits), 2)}</b>", unsafe_allow_html=True)
        col_res_5, col_res_6, col_res_7, col_res_8 = st.columns(4)
        col_res_5.markdown(f"<b>Avg execution quality entry: {round(entries.execution_quality.sum() / (len(entries)), 2)}</b>", unsafe_allow_html=True)
        col_res_6.markdown(f"<b>Avg execution quality exit: {round(exits.execution_quality.sum()/ len(exits), 2)}</b>", unsafe_allow_html=True)
        col_res_7.markdown(f"<b>Total volume traded: {total_volume_traded}</b>", unsafe_allow_html=True)
        col_res_8.markdown(f"<b>Total volume traded in coin: {total_volume_traded_in_token}</b>", unsafe_allow_html=True)


        st.markdown("<h3 style='text-align: center; color: grey;'>Descriptive statics of Simulated Executions in the given period</h3>", unsafe_allow_html=True)
        ...
        st.download_button("Press to Download", simulated_executions.to_csv(), f"Sim_{strategy}_from_{date_range[0]}_to_{date_range[1]}.csv", "text/csv")
        ...
        if False:
            ...
            fig2.add_trace(go.Histogram(
                x=simulated_cancellations_exit["timestamp_cancelled"] - simulated_cancellations_exit["timestamp_posted"],
